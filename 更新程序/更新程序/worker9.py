# ==========================================
# worker9 → 4001-4500【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 9
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 4001) & (df["id"] <= 4500)]

    print(f"📌 Worker9 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker9 完成 → {code} {code_name}"
        try:
