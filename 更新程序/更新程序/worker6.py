# ==========================================
# worker6 → 2501-3000【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 6
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 2501) & (df["id"] <= 3000)]

    print(f"📌 Worker6 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker6 完成 → {code} {code_name}"
        try:
