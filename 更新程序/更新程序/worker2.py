# ==========================================
# worker2 → 501-1000【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 2
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 501) & (df["id"] <= 1000)]

    print(f"📌 Worker2 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker2 完成 → {code} {code_name}"
        try:
