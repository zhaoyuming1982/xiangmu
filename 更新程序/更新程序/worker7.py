# ==========================================
# worker7 → 3001-3500【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 7
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 3001) & (df["id"] <= 3500)]

    print(f"📌 Worker7 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker7 完成 → {code} {code_name}"
        try:
