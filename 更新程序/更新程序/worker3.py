# ==========================================
# worker3 → 1001-1500【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 3
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 1001) & (df["id"] <= 1500)]

    print(f"📌 Worker3 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker3 完成 → {code} {code_name}"
        try:
