# ==========================================
# worker8 → 3501-4000【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 8
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 3501) & (df["id"] <= 4000)]

    print(f"📌 Worker8 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker8 完成 → {code} {code_name}"
        try:
