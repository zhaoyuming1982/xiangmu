# ==========================================
# worker5 → 2001-2500【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 5
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 2001) & (df["id"] <= 2500)]

    print(f"📌 Worker5 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker5 完成 → {code} {code_name}"
        try:
