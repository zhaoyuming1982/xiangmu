...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 1) & (df["id"] <= 500)]

    print(f"📌 Worker1 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        # 日志
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker1 完成 → {code} {code_name}"
        try:
