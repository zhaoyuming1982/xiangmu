# ==========================================
# worker4 → 1501-2000【防踢线、自动重登、绝不卡死版】
# ==========================================
WORKER_ID = 4
...
def download_data_by_task():
    ...
    df_worker = df[(df["id"] >= 1501) & (df["id"] <= 2000)]

    print(f"📌 Worker4 分配到：{len(df_worker)} 只股票")
    ...
    for _, row in df_worker.iterrows():
        ...
        msg = f"[{time.strftime('%H:%M:%S')}] 🟢 Worker4 完成 → {code} {code_name}"
        try:
